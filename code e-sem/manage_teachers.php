...
if($user_data['accesslevel'] == 0)
  echo "<center><h1>Teacher Management</h1>";

?>

<button id="emf" class="btn btn-primary">Add New</button>
<br><br>
<button id="emf2" class="btn btn-primary">Enabled\Disabled</button>

<br>
<div class="container">
<form class="form-horizontal" id="formadd" action="add_teacher.php" method="post">
		<h1>Add New</h1>
...
echo "<b><font class=\"anenf\" size=\"6\">Disabled</font></b>";
...
{
	$results_id=$con->prepare("SELECT * FROM users WHERE aprooved=0 AND accesslevel=1");
	$results_id->execute();
...
?>
			...
			?>
				...
				<a href="edituser.php?userid=<?php echo "$id"; ?>" class="btn btn-primary" role="button">Edit</a>
			</div></td>
			<?php
			  }
			?>
...
echo "<b><font class=\"enenf\" size=\"6\">Enabled</font></b> <br>";
...
{
	$results_id=$con->prepare("SELECT * FROM users WHERE aprooved=1 AND accesslevel=1");
	$results_id->execute();
